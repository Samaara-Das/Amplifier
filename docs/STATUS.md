...
- **48 done** · **1 partial** · **3 pending** · **22 deferred** · 0 in-progress · **74 total**

> **Phase D #70 SHIPPED 2026-05-01 01:55 IST.** BYOK — companies bring their own Gemini/Mistral/Groq API keys via Settings page. New `company_api_keys` table (Alembic `b1c2d3e4f5a6`, applied to prod), AES-encrypted via existing `server/app/utils/crypto.py`. New `server/app/services/api_keys.py` provides `resolve_api_key()` (company key → env var fallback) + `call_with_byok_fallback()` (auth-error retry path). Wired into `quality_gate.ai_review_campaign` + `campaign_wizard` AI calls (threaded `db: AsyncSession | None = None` through, backwards-compat default). UI: 3 inputs (Gemini/Mistral/Groq) + Test button per provider; **never echoes plaintext** — shows "•••••• (configured — re-enter to overwrite)" or "Not set". 15 new tests cover encryption round-trip, all resolve paths, auth-error retry, settings POST upsert, ai_review_campaign wiring. **303/303 pytest green** (288 + 15). Deployed to api.pointcapitalis.com — `company_api_keys` table created, both services active. Branch merged to flask-user-app as `1d1d7d5`. **PHASE D NOW COMPLETE for active features** (#66 ✅, #67 ✅, #70 ✅; #68 partial — Patchright shipped, installer build awaits v1.0.0 release tag).

> **Phase D #68 PARTIAL 2026-05-01 01:35 IST.** Stealth + packaging — verified ACs: AC-1 (Patchright drop-in: 22 files swapped from `playwright.async_api` → `patchright.async_api`, --disable-blink-features flag removed from 11 sites, 288/288 pytest green, real Chromium launched), AC-2 (CreepJS reports `0% stealth` + `0% headless` — vanilla Playwright fails this badly, Patchright stealth invisible), AC-11/12 (tauri-app/ absent, no PyInstaller artifacts). Code shipped, awaits release-prep verification: AC-3 (Nuitka Windows build), AC-5 (Inno Setup installer), AC-7 (auto-update), AC-8 (GHA matrix). Deferred (no env): AC-4/AC-6 (Mac builds). Scaffolding: scripts/build/{spec.json, build_windows.py, build_mac.py, installer/{windows.iss, eula.rtf, postinstall.sh}, build_*_installer.{ps1,sh}}, scripts/utils/auto_update.py (166 LOC, GitHub Releases polling, urllib stdlib), .github/workflows/{release.yml, build-test.yml}, tray "Check for Updates" menu item, BackgroundAgent 24h update cycle. **Live verification deferred to v1.0.0 release tag** — autonomous loop policy forbids tag pushes. Branch merged to flask-user-app as `ed93aaf`.
