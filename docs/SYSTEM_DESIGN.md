...
## Decision 5: Content Generation via Claude CLI

**Problem:** How is social media content generated?

**Decision:** Content is generated by invoking Claude CLI as a subprocess from PowerShell scripts. Two scripts exist:
- `scripts/generate.ps1` -- Personal brand content. Per-slot generation with pillar rotation, CTA rotation, and legal disclaimers.
- `scripts/generate_campaign.ps1` -- Campaign content. Takes a campaign JSON file (brief, assets, content guidance) and produces platform-specific content.

Both scripts call `claude --dangerously-skip-permissions -p "<prompt>"` and write output as JSON files.

**Implications:**
- **Windows-only** -- PowerShell is required. The generation scripts use PowerShell-specific features.
- **User bears AI cost** -- Each user needs a Claude CLI subscription. Generation cost is per-invocation.
- **Prompt quality matters** -- Content quality depends entirely on the prompt engineering in the PowerShell scripts. The prompt includes brand voice from `config/content-templates.md`.
- **No streaming** -- Generation is synchronous with a 300-second timeout (5 minutes). Output is the path to the generated JSON file.
- **AI rewrite in dashboard** -- The review dashboard (`review_dashboard.py`) also uses Claude CLI for inline rewrites, spawning a subprocess with a 120-second timeout.

**Alternative Considered:** Direct Claude API calls from Python. Rejected because: (a) Claude CLI handles auth and model selection transparently, (b) the `--dangerously-skip-permissions` flag enables file writing which simplifies the generation pipeline, (c) keeps the generation logic in a single prompt template rather than spread across API call code.
...
3. **Budget Check** -- Campaigns with `budget_remaining <= 0` are excluded.
4. **Already Assigned** -- Campaigns already assigned to this user are excluded.
