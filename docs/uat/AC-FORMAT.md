...
- `AMPLIFIER_UAT_PROFILE_REFRESH_NOW=1` — shortens `PROFILE_REFRESH_INTERVAL` in `background_agent.py` from 604800s (7 days) to 30s, so the daemon's automatic profile-refresh loop fires every 30s during UAT instead of once per week. Used by Task #86 AC8.
